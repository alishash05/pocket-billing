...
const onChange = (e) => {
    ...
    const { id, value } = e.target;
    if (id === 'username') setUserInput(value);
    if (id === 'password') setPassInput(value);
  }
  ...
  const Login = (e) => {
    ...
    if (storedData && storedData.username === userInput && storedData.password === passInput) {
      navigate('/dashboard');
    } else {
		...
		<div className={classes.loginPage}>
			<div className={classes.form}>
				<form className={classes.loginForm} onSubmit={Login}>  
					...
					<p className={classes.message}>Not registered? <a href="/sign-in">Create an account</a></p>
				</form> 
			</div>
		</div>
	</div>
